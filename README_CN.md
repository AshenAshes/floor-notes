...
打开楼层视图时，原生大纲中的 Floor 和 Reply 会显示正文摘要，回复继续嵌套在所属楼层下。摘要不带楼层号，长文本单行省略；原生搜索支持摘要，点击支持跨页定位。保留原生展开与折叠状态，切回普通页面后恢复原生标题。此适配基于 Obsidian 1.13.7 的内部大纲结构；不兼容的结构会保持原生行为。
